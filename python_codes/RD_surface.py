...
# surface ligands)
Ai = np.array([[3247, 3374]])
Bi = np.array([[3637, 4591]])
Ci = np.array([[4272, 4440]])
Di = np.array([[4596, 3975]])
Ei = np.array([[2678, 2692]])
Fi = np.array([[4573, 2745]])

# Second group of vertices (formed by one 6MR window each and identified by the three existing
# surface ligands)
Gi = np.array([[4220, 4559, 4558]])
Hi = np.array([[3503, 2787, 3813]])
Ii = np.array([[4564, 3755, 2313]])
Ji = np.array([[3802, 4157, 4565]])
Ki = np.array([[4570, 4588, 4212]])
Li = np.array([[4580, 2597, 4555]])
Mi = np.array([[3113, 3052, 3863]])
Ni = np.array([[3006, 3064, 4258]])
...
# for the environment of the facets. All of them will have histograms 12 angs long. The bins
...
for loop_0 in range (0, number_of_configurations_considered):
    ...
    grp_edges = np.concatenate((AM, AN, AK, AL, FN, FM, FG, FH, BJ, BH, BG, BI, EJ, EK, EI, EL, DM, DK, DH, DJ, CL, CI, CN, CG), axis = 0)
    ...
    grp_facets = np.concatenate((MN, MK, LK, LN, NG, MH, GH, JH, GI, IJ, JK, IL), axis = 0)
